...
struct RejectDiagnostic {
    std::string reason;
    uint32_t rejectCursor = 0;
    uint8_t rejectCmd = 0;
    bool rejectCmdValid = false;
};
...
public:
    ...
    void run() {
        ...
        writeRejectedCandidatesTsv(joinPath(_options.outDir, "rejected_candidates_with_reasons.tsv"));
        writeRejectedCandidatesJson(joinPath(_options.outDir, "rejected_candidates_with_reasons.json"));
        writeRejectedReasonSummaryTxt(joinPath(_options.outDir, "rejected_reason_summary.txt"));
        writeRejectedReasonSummaryJson(joinPath(_options.outDir, "rejected_reason_summary.json"));
        if (kEnableMissingDialogueAudit) {
...
private:
    ...
    std::vector<RejectedCandidate> _rejectedDetails;
    ...
    RejectDiagnostic makeRejectDiagnostic(const std::string &reason, uint32_t rejectCursor,
                                          bool rejectCmdValid = false, uint8_t rejectCmd = 0) {
        RejectDiagnostic diagnostic;
        diagnostic.reason = reason;
        diagnostic.rejectCursor = rejectCursor;
        diagnostic.rejectCmdValid = rejectCmdValid;
        diagnostic.rejectCmd = rejectCmd;
        return diagnostic;
    }

    bool scummResStrLenWithReason(const ScriptSource &src, uint32_t start, uint32_t &outLen, std::string &reason) {
        if (start >= src.len) {
            reason = "start_oob";
            return false;
        }
        ...
        while (cursor < src.len) {
            ...
            if (chr == 0xFF) {
                if (cursor >= src.len) {
                    reason = "truncated_escape_opcode";
                    return false;
                }
                const uint8_t code = data[base + cursor];
                ++cursor;
        ...
        reason = "no_string_terminator";
        return false;
    }

    RejectDiagnostic diagnoseRejectedPrintCandidate(const ScriptSource &src, uint32_t opcodeOffset) {
        if (opcodeOffset >= src.len) {
            return makeRejectDiagnostic("opcode_offset_oob", opcodeOffset);
        }

        const std::vector<uint8_t> &data = *src.buffer;
        const uint32_t base = src.baseOffsetFile;
        const uint8_t opcode = data[base + opcodeOffset];
        ...
        if (opcode == 0x14 || opcode == 0x94) {
            if (opcode & PARAM_1) {
                if (cursor + 2 > src.len) {
                    return makeRejectDiagnostic("actor_var_arg_oob", cursor);
                }
                cursor += 2;
            } else {
                if (cursor + 1 > src.len) {
                    return makeRejectDiagnostic("actor_direct_arg_oob", cursor);
                }
                cursor += 1;
            }
        }
        ...
        while (cursor < src.len) {
            const uint8_t cmd = data[base + cursor];
            ++cursor;

            if (cmd == 0xFF) {
                return makeRejectDiagnostic("terminator_before_text", cursor - 1, true, cmd);
            }

            switch (cmd & 0x0F) {
            case 0:
                if (!consumeWordArg(cursor, src) || !consumeWordArg(cursor, src)) {
                    return makeRejectDiagnostic("cmd0_arg_oob", cursor - 1, true, cmd);
                }
                break;
            case 1:
                if (!consumeByteArg(cmd, PARAM_1, cursor, src)) {
                    return makeRejectDiagnostic("cmd1_arg_oob", cursor - 1, true, cmd);
                }
                break;
            case 2:
                if (!consumeWordArg(cursor, src)) {
                    return makeRejectDiagnostic("cmd2_arg_oob", cursor - 1, true, cmd);
                }
                break;
            case 3:
                if (!consumeWordArg(cursor, src) || !consumeWordArg(cursor, src)) {
                    return makeRejectDiagnostic("cmd3_arg_oob", cursor - 1, true, cmd);
                }
                break;
            case 4:
                break;
            case 6:
                if (!consumeWordArg(cursor, src)) {
                    return makeRejectDiagnostic("cmd6_arg_oob", cursor - 1, true, cmd);
                }
                break;
            case 7:
                break;
            case 8:
                if (!consumeWordArg(cursor, src) || !consumeWordArg(cursor, src)) {
                    return makeRejectDiagnostic("cmd8_arg_oob", cursor - 1, true, cmd);
                }
                break;
            case 15: {
                uint32_t msgLen = 0;
                std::string lengthReason;
                if (!scummResStrLenWithReason(src, cursor, msgLen, lengthReason)) {
                    return makeRejectDiagnostic("text_len_" + lengthReason, cursor, true, cmd);
                }
                return makeRejectDiagnostic("diagnostic_ok_unexpected", cursor, true, cmd);
            }
            default:
                return makeRejectDiagnostic("unsupported_cmd_nibble", cursor - 1, true, cmd);
            }
        }

        return makeRejectDiagnostic("reached_end_without_text", cursor);
    }
        ...
        for (const ScriptSource &src : _sources) {
            ...
            for (uint32_t off = 0; off < src.len; ++off) {
                ...
                if (!parsed.ok) {
                    ...
                    rejected.candidateIndex = _candidateOpcodes;
                    rejected.lflFile = src.lflFile;
                    rejected.sourceKind = src.kind;
                    rejected.roomId = src.roomId;
                    rejected.scriptId = src.scriptId;
                    rejected.objectId = src.objectId;
                    rejected.verbId = src.verbId;
                    rejected.sourceBaseOffset = src.baseOffsetFile;
                    rejected.sourceLen = src.len;
                    rejected.opcode = opcode;
                    rejected.opcodeOffsetInSource = off;
                    rejected.diagnostic = diagnoseRejectedPrintCandidate(src, off);
                    _rejectedDetails.push_back(std::move(rejected));
                    continue;
                }
    ...
    void writeRejectedCandidatesTsv(const std::string &path) {
        std::ofstream out(path, std::ios::binary);
        if (!out) {
            fail(path, "failed to open output file");
        }

        out << "candidate_idx\tlfl_file\tsource_kind\troom\tscript\tobject\tverb"
            << "\tsource_base\tsource_len\topcode\topcode_offset\treason\treject_cursor\treject_cmd\n";
        for (const RejectedCandidate &rejected : _rejectedDetails) {
            out << rejected.candidateIndex << "\t"
                << rejected.lflFile << "\t"
                << sourceKindToString(rejected.sourceKind) << "\t"
                << rejected.roomId << "\t"
                << rejected.scriptId << "\t"
                << rejected.objectId << "\t"
                << rejected.verbId << "\t"
                << rejected.sourceBaseOffset << "\t"
                << rejected.sourceLen << "\t"
                << "0x" << hexByte(rejected.opcode) << "\t"
                << rejected.opcodeOffsetInSource << "\t"
                << rejected.diagnostic.reason << "\t"
                << rejected.diagnostic.rejectCursor << "\t";
            if (rejected.diagnostic.rejectCmdValid) {
                out << "0x" << hexByte(rejected.diagnostic.rejectCmd);
            }
            out << "\n";
        }
    }

    void writeRejectedCandidatesJson(const std::string &path) {
        std::ofstream out(path, std::ios::binary);
        if (!out) {
            fail(path, "failed to open output file");
        }
        ...
        out << "[\n";
        for (size_t i = 0; i < _rejectedDetails.size(); ++i) {
            const RejectedCandidate &rejected = _rejectedDetails[i];
            out << "  {";
            out << "\"candidate_idx\":" << rejected.candidateIndex;
            out << ",\"lfl_file\":\"" << jsonEscape(rejected.lflFile) << "\"";
            out << ",\"source_kind\":\"" << sourceKindToString(rejected.sourceKind) << "\"";
            out << ",\"room\":" << rejected.roomId;
            out << ",\"script\":" << rejected.scriptId;
            out << ",\"object\":" << rejected.objectId;
            out << ",\"verb\":" << rejected.verbId;
            out << ",\"source_base\":" << rejected.sourceBaseOffset;
            out << ",\"source_len\":" << rejected.sourceLen;
            out << ",\"opcode\":\"0x" << hexByte(rejected.opcode) << "\"";
            out << ",\"opcode_offset\":" << rejected.opcodeOffsetInSource;
            out << ",\"reason\":\"" << jsonEscape(rejected.diagnostic.reason) << "\"";
            out << ",\"reject_cursor\":" << rejected.diagnostic.rejectCursor;
            if (rejected.diagnostic.rejectCmdValid) {
                out << ",\"reject_cmd\":\"0x" << hexByte(rejected.diagnostic.rejectCmd) << "\"";
            }
            out << "}";
            if (i + 1 != _rejectedDetails.size()) {
                out << ",";
            }
            out << "\n";
        }
    ...
    void writeRejectedReasonSummaryTxt(const std::string &path) {
        std::ofstream out(path, std::ios::binary);
        if (!out) {
            fail(path, "failed to open output file");
        }
        ...
        for (const RejectedCandidate &rejected : _rejectedDetails) {
            ++byReason[rejected.diagnostic.reason];
        }

        out << "total_rejected\t" << _rejectedDetails.size() << "\n\n";
        out << "reason\tcount\n";
        for (const auto &entry : byReason) {
            out << entry.first << "\t" << entry.second << "\n";
        }
    }

    void writeRejectedReasonSummaryJson(const std::string &path) {
        std::ofstream out(path, std::ios::binary);
        if (!out) {
            fail(path, "failed to open output file");
        }
        ...
        out << "  \"total_rejected\": " << _rejectedDetails.size() << ",\n";
        out << "  \"by_reason\": {\n";
        for (auto it = byReason.begin(); it != byReason.end(); ++it) {
            out << "    \"" << jsonEscape(it->first) << "\": " << it->second;
            if (std::next(it) != byReason.end()) {
                out << ",";
            }
            out << "\n";
        }
    ...
    void writeStats(const std::string &path) {
        ...
        std::map<std::string, size_t> byRejectReason;
        ...
        for (const RejectedCandidate &rejected : _rejectedDetails) {
            ++byRejectReason[rejected.diagnostic.reason];
        }
        ...
        out << "  \"rejected_candidates_with_reasons\": " << _rejectedDetails.size() << ",\n";

        out << "  \"counts_by_reject_reason\": {\n";
        for (auto it = byRejectReason.begin(); it != byRejectReason.end(); ++it) {
            out << "    \"" << jsonEscape(it->first) << "\": " << it->second;
            if (std::next(it) != byRejectReason.end()) {
                out << ",";
            }
            out << "\n";
        }
